...
# Mapping of ISO 639-1 language codes to full language names for LLM prompts
LANGUAGE_NAMES = {
...
def get_language_name(code, default='the same language as the transcript'):
    """Convert a 2-letter ISO language code to a human-readable name."""
    return LANGUAGE_NAMES.get(code, default)

# Localized section headers for summary output
LANGUAGE_HEADERS = {
    'hi': {'summary': 'सारांश', 'takeaways': 'मुख्य बिंदु'},
    'mr': {'summary': 'सारांश', 'takeaways': 'मुख्य मुद्दे'},
    'bn': {'summary': 'সারাংশ', 'takeaways': 'মূল বিষয়'},
    'ta': {'summary': 'சுருக்கம்', 'takeaways': 'முக்கிய குறிப்புகள்'},
    'te': {'summary': 'సారాంశం', 'takeaways': 'ముఖ్య అంశాలు'},
    'gu': {'summary': 'સારાંશ', 'takeaways': 'મુખ્ય મુદ્દાઓ'},
    'kn': {'summary': 'ಸಾರಾಂಶ', 'takeaways': 'ಪ್ರಮುಖ ಅಂಶಗಳು'},
    'ml': {'summary': 'സംഗ്രഹം', 'takeaways': 'പ്രധാന കാര്യങ്ങൾ'},
    'pa': {'summary': 'ਸਾਰ', 'takeaways': 'ਮੁੱਖ ਨੁਕਤੇ'},
    'ur': {'summary': 'خلاصہ', 'takeaways': 'اہم نکات'},
}

def get_section_headers(lang_code):
    """Return localized section headers. Falls back to English."""
    return LANGUAGE_HEADERS.get(lang_code, {'summary': 'Summary', 'takeaways': 'Key Takeaways'})
...
def get_video_title(video_id):
    """Fetch the YouTube video title using the oEmbed API (no API key required).
    Returns 'Unknown Title' on any failure.
    """
    try:
        ...
        print(f"[PodcastAI] Could not fetch video title for {video_id}: {e}")
...
async def summarize_chunk(chunk_text, index, language_name='English'):
    try:
        llm = ChatOllama(
        ...
        # If the language is NOT English, use a stronger language-forcing prompt
        if language_name != 'English':
            prompt = PromptTemplate.from_template(
                """IMPORTANT: Your ENTIRE response MUST be written in {language_name} only.
Do NOT write anything in English. Every word of your output must be in {language_name}.

Below is a transcript chunk written in {language_name}. Summarize it into 2-3 concise bullet points in {language_name}.

Transcript chunk {num}:
{text}

Write your bullet points in {language_name} only:""")
        else:
            prompt = PromptTemplate.from_template(
                """Summarize the following transcript chunk into 2-3 concise bullet points.

Chunk {num}:
{text}

Provide ONLY the bullet points, no extra text.""")
...
async def generate_distributed_summary_async(text):
    try:
        # Detect language from transcript so all outputs match it
        detected_lang = detect_language(text)
        language_name = get_language_name(detected_lang)
        headers = get_section_headers(detected_lang)

        print(f"[PodcastAI] Detected transcript language: {detected_lang} ({language_name})")
        ...
        for i, doc in enumerate(docs):
            tasks.append(
                summarize_chunk(doc.page_content, i, language_name=language_name)
            )
        ...
        # Include a prefix of the original transcript to anchor the model's language
        transcript_prefix = text[:500] if len(text) > 500 else text
        ...
        # Use different prompts for non-English vs English to maximize compliance
        if detected_lang != 'en':
            final_prompt = PromptTemplate.from_template(
                """CRITICAL INSTRUCTION: Your ENTIRE response MUST be in {language_name}. Do NOT use English at all.

Here is a sample from the original transcript in {language_name}:
---
{transcript_sample}
---

Below are summarized notes from this transcript. Combine them into a final summary.
Your response MUST follow this EXACT format (keep the ### headers as shown):

### {summary_header}
(Write one concise paragraph in {language_name} summarizing the entire content.)

### {takeaways_header}
(Write 10-15 bullet points in {language_name}. Each point starts with '- '.)

Summarized notes:
{text}

REMEMBER: Write EVERYTHING in {language_name}. Not a single word in English.""")

            final_response = await llm.ainvoke(final_prompt.format(
                text=combined,
                language_name=language_name,
                transcript_sample=transcript_prefix,
                summary_header=headers['summary'],
                takeaways_header=headers['takeaways']
            ))
        else:
            final_prompt = PromptTemplate.from_template(
...
### Key Takeaways
(Exactly 10-15 bullet points containing the most important insights. Each point starts with '- '.)
...
async def analyze_sentiment_emotion_async(text, text_type="transcript"):
    ...
    try:
        ...
        prompt = PromptTemplate.from_template("""Analyze the sentiment and emotion of the following {text_type} text.

Rules:
1. Return ONLY valid JSON. Do NOT include any text, explanation, or markdown before or after the JSON.
2. Use EXACTLY the format shown below.
3. "sentiment" must be one of: "Positive", "Negative", "Neutral".
4. "emotion" must be one of: "Excited", "Serious", "Motivational", "Sad", "Informative", "Angry", "Neutral".
5. Scores must be integers between 0 and 100.

Required JSON format:
{{"sentiment": "Positive", "sentiment_score": 75, "emotion": "Informative", "emotion_confidence": 80}}
...
async def calculate_accuracy_scores_async(full_text, summary):
    ...
    try:
        ...
        transcription_prompt = PromptTemplate.from_template("""Rate the quality and completeness of this transcript on a scale of 0 to 100.
Consider grammar, punctuation, sentence structure, and comprehensiveness.

Rules:
- Respond with ONLY a single integer between 0 and 100.
- Do NOT include any explanation, words, symbols, or punctuation — ONLY the number.
...
{text}""")
        ...
        # Summary confidence
        summary_prompt = PromptTemplate.from_template("""Rate how well this summary captures the original transcript content on a scale of 0 to 100.
...
def generate_topics_qa(full_text, language='en'):
    ...
    try:
        ...
        # Use a plain string prompt (NOT PromptTemplate) to avoid double-brace escaping issues
        prompt_text = f"""Analyze this podcast transcript and extract exactly 5 key topics discussed.
For each topic, generate 2 relevant questions and their answers based ONLY on the transcript content.

Transcript:
{analysis_text}

RULES:
1. Write ALL topic names, questions, and answers in {language_name}.
2. Return ONLY a valid JSON array. No explanation, no markdown, no extra text.
3. Example format:
[
  {{"topic": "Topic 1", "questions": [{{"q": "Question?", "a": "Answer"}}, {{"q": "Question?", "a": "Answer"}}]}},
  {{"topic": "Topic 2", "questions": [{{"q": "Question?", "a": "Answer"}}, {{"q": "Question?", "a": "Answer"}}]}}
]
...
Return ONLY the JSON array:"""
        ...
        print(f"[PodcastAI] Topics: First attempt failed to parse JSON. Retrying with simpler prompt...")
        simple_prompt = f"""Read this transcript and list 3 main topics with 1 question and answer each.

Transcript (first 2000 chars):
{analysis_text[:2000]}

Return ONLY valid JSON like this:
[{{"topic": "Topic", "questions": [{{"q": "Question?", "a": "Answer"}}]}}]

JSON:"""

        response2 = llm.invoke(simple_prompt)
        topics = _parse_topics_json(response2.content.strip())
        return topics
...
def _parse_topics_json(response_text):
    """Parse and validate topics JSON from LLM response. Returns list or empty list."""
    try:
        ...
        if json_match:
            ...
            if valid_topics:
                print(f"[PodcastAI] Topics: Successfully parsed {len(valid_topics)} topics")
            return valid_topics
        else:
            print(f"[PodcastAI] Topics: No JSON array found in response. Raw response (first 300 chars): {response_text[:300]}")
            return []

    except json.JSONDecodeError as e:
        print(f"[PodcastAI] Topics: JSON parse error: {e}. Raw response (first 300 chars): {response_text[:300]}")
        return []
    except Exception as e:
        print(f"[PodcastAI] Topics: Unexpected parse error: {e}")
        return []
...
def generate_pdf(video_id, record):
    ...
    try:
        ...
        if video_title:
            ...
            elements.append(Paragraph(f"<b>Video Title:</b>  {safe_vt}", meta_st))
        if video_url:
        ...
        if any(kw in raw_summary for kw in
               ['### Summary', '### Key Takeaways', 'Summary', 'Key Takeaways',
                '### \u0938\u093e\u0930\u093e\u0902\u0936', '### \u092e\u0941\u0916\u094d\u092f \u092c\u093f\u0902\u0926\u0941', '### \u092e\u0941\u0916\u094d\u092f \u092e\u0941\u0926\u094d\u0926\u0947',
                '\u0938\u093e\u0930\u093e\u0902\u0936', '\u092e\u0941\u0916\u094d\u092f \u092c\u093f\u0902\u0926\u0941', '\u092e\u0941\u0916\u094d\u092f \u092e\u0941\u0926\u094d\u0926\u0947',
                '\u09b8\u09be\u09b0\u09be\u0982\u09b6', '\u09ae\u09c2\u09b2 \u09ac\u09bf\u09b7\u09af\u09bc',
                '\u0b9a\u0bc1\u0bb0\u0bc1\u0b95\u0bcd\u0b95\u0bae\u0bcd', '\u0bae\u0bc1\u0b95\u0bcd\u0b95\u0bbf\u0baf \u0b95\u0bc1\u0bb1\u0bbf\u0baa\u0bcd\u0baa\u0bc1\u0b95\u0bb3\u0bcd',
                '\u0c38\u0c3e\u0c30\u0c3e\u0c02\u0c36\u0c02', '\u0c2e\u0c41\u0c16\u0c4d\u0c2f \u0c05\u0c02\u0c36\u0c3e\u0c32\u0c41',
...
def dashboard():
    if request.method == 'POST':
        ...
        if cached:
            # Backfill video_title for older records that don't have it
            if not cached.get('video_title'):
...
def chat():
    ...
    # Get response with confidence score using full transcript + video title
    result = chat_service.get_chat_response(full_text, summary, message, language=lang_code, video_title=video_title)
...
def history():
    ...
    # Enrich each history item with the video title from summaries
    for item in history:
        vid = item.get('video_id')
        if vid:
            summary_rec = summaries_collection.find_one({'video_id': vid}, {'video_title': 1})
            item['video_title'] = (summary_rec or {}).get('video_title', '')
